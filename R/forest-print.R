...
format.timbr_forest <- function(x, ...) {
  setup <- tbl_format_setup(x, ..., n = nrow(x$roots))
...
tbl_sum.timbr_forest <- function(x, ...) {
  ...
  c(
    `A forest` = paste(
      big_mark(size_nodes),
      plural("node", size_nodes),
      "and",
      big_mark(size_features),
      plural("feature", size_features)
    ),
    group_sum,
    Trees = "",
    tree_forest(x)
...
tbl_format_header.timbr_forest <- function(x, setup, ...) {
  ...
  nms <- names(tbl_sum)
  header <- paste0(
    dplyr::if_else(nms == "", "", pillar::align(paste0(nms, ": "))),
    dplyr::if_else(nms == "", paste0("  ", tbl_sum), tbl_sum)
  )
...
tbl_format_body.timbr_forest <- function(x, setup, ...) {
  ...
  tbl_format_body(root_nodes, setup = pillar::tbl_format_setup(root_nodes))
}

#' @export
tbl_format_footer.timbr_forest <- function(x, setup, ...) {
  root_nodes <- get_root_nodes(x)
  tbl_format_footer(root_nodes, setup = pillar::tbl_format_setup(root_nodes))
}
...
tree_forest <- function(x) {
  ...
  group_rle <- vec_group_rle(data_frame(
  ...
  nodes <- data_frame(
    . = node(name = node_names, value = NA_character_),
    name = node_names,
    size = field(group_rle, "length"),
    children = list(character())
  )
  edges <- data_frame(from = parent_node_ids, to = seq_along) |>
    ...
    traverse(\(x, y) {
      ...
      children <- list(y$name, y$size, y$children, is_last) |>
        purrr::pmap(\(name, size, children, is_last) {
          ...
          c(
            paste0(prefix_head, name, " [", big_mark(size), "]"),
            paste0(prefix_tail, children, recycle0 = TRUE)
          )
        })
  ...
  list(root_nodes$name, root_nodes$size, root_nodes$children) |>
    purrr::pmap(\(name, size, children) {
      c(paste0(name, " [", big_mark(size), "]"), children)
    }) |>
    list_unchop()
}

...
get_root_nodes <- function(x) {
  roots <- x$roots
  nodes <- get_nodes(x)
  root_nodes <- tibble::tibble(drop_node(roots), vec_slice(nodes, x$roots$.))
...
get_parent_node_ids <- function(x) {
  ...
  vec_slice(edges$from, vec_match(vec_seq_along(nodes), edges$to))
}
...
box_chars <- function() {
  # nocov start
  # Coverage of this branch depends on whether the terminal running the
  # tests reports UTF-8 support, so only one side is ever hit per platform.
